...
public class TestService(IPersonaRepository personaRepo, IRefinementService refinementService): ITestService
{
    public async Task TestPersonaRefinement(CancellationToken ct)
    {
        ...
        var result = await refinementService.RefinePersona(persona, ct);
