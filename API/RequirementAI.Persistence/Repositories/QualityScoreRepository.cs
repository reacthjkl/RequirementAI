...
public class QualityScoreRepository(RequirementAIContext context): IQualityScoreRepository
{
    ...
    public async Task<List<PersonaQualityScore>> GetPersonaQualityScores(Guid personaId, Guid organizationId, CancellationToken ct)
    {
        return await context.PersonaQualityScores
            .Where(x => x.PersonaId == personaId && x.Persona.Project.OrganizationId == organizationId)
            .OrderByDescending(x => x.CreatedAt)
            .ToListAsync(ct);
    }
    ...
    public async Task<List<ScenarioQualityScore>> GetScenarioQualityScores(Guid scenarioId, Guid organizationId, CancellationToken ct)
    {
        return await context.ScenarioQualityScores
            .Where(x => x.ScenarioId == scenarioId && x.Scenario.Persona.Project.OrganizationId == organizationId)
            .OrderByDescending(x => x.CreatedAt)
            .ToListAsync(ct);
    }
    ...
    public async Task<List<UserStoryQualityScore>> GetUserStoryQualityScores(Guid userStoryId, Guid organizationId, CancellationToken ct)
    {
        return await context.UserStoryQualityScores
            .Where(x => x.UserStoryId == userStoryId &&
                        x.UserStory.Scenario.Persona.Project.OrganizationId == organizationId)
            .OrderByDescending(x => x.CreatedAt)
            .ToListAsync(ct);
    }
    ...
    public async Task<PersonaQualityScore?> GetLatestPersonaQualityScore(Guid personaId, Guid organizationId, CancellationToken ct)
    {
        return await context.PersonaQualityScores
            .Where(x => x.PersonaId == personaId && x.Persona.Project.OrganizationId == organizationId)
            .OrderByDescending(x => x.CreatedAt)
    ...
    public async Task<ScenarioQualityScore?> GetLatestScenarioQualityScore(Guid scenarioId, Guid organizationId, CancellationToken ct)
    {
        return await context.ScenarioQualityScores
            .Where(x => x.ScenarioId == scenarioId && x.Scenario.Persona.Project.OrganizationId == organizationId)
            .OrderByDescending(x => x.CreatedAt)
    ...
    public async Task<UserStoryQualityScore?> GetLatestUserStoryQualityScore(Guid userStoryId, Guid organizationId, CancellationToken ct)
    {
        return await context.UserStoryQualityScores
            .Where(x => x.UserStoryId == userStoryId &&
                        x.UserStory.Scenario.Persona.Project.OrganizationId == organizationId)
    ...
    public async Task<List<QualityScoreTrendProjection>> GetAllQualityScoresByProjectId(Guid projectId, Guid organizationId, CancellationToken ct)
    {
        var personaScores = context.PersonaQualityScores
            .Where(x => x.Persona.ProjectId == projectId &&
                        x.Persona.Project.OrganizationId == organizationId)
            .Select(x => new QualityScoreTrendProjection
            {
                OverallScore = x.OverallScore,
                CreatedAt = x.CreatedAt,
        ...
        var scenarioScores = context.ScenarioQualityScores
            .Where(x => x.Scenario.Persona.ProjectId == projectId &&
                        x.Scenario.Persona.Project.OrganizationId == organizationId)
            .Select(x => new QualityScoreTrendProjection
            {
                OverallScore = x.OverallScore,
                CreatedAt = x.CreatedAt,
    ...
    public async Task<List<PersonaQualityScore>> GetLatestPersonaQualityScoresByProjectId(Guid projectId, Guid organizationId, CancellationToken ct)
    {
        return await context.PersonaQualityScores
            .Include(x => x.Persona)
            .Where(x => x.Persona.ProjectId == projectId &&
    ...
    public async Task<List<ScenarioQualityScore>> GetLatestScenarioQualityScoresByProjectId(Guid projectId, Guid organizationId, CancellationToken ct)
    {
        return await context.ScenarioQualityScores
            .Include(x => x.Scenario)
            .ThenInclude(x => x.Persona)
    ...
    public async Task<List<UserStoryQualityScore>> GetLatestUserStoryQualityScoresByProjectId(Guid projectId, Guid organizationId, CancellationToken ct)
    {
        return await context.UserStoryQualityScores
            .Include(x => x.UserStory)
            .ThenInclude(x => x.Scenario)
