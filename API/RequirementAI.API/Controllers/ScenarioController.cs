...
public class ScenarioController(IScenarioService scenarioService): RequirementAIControllerBase
{
    ...
    {
        var scenario = await scenarioService.GetById(id, ct);
    ...
    {
        var scenarios = await scenarioService.GetByPersonaId(personaId, ct);
    ...
    {
        var created = await scenarioService.Create(scenario, ct);
    ...
    {
        await scenarioService.Update(scenario, ct);
    ...
    {
        await scenarioService.Delete(id, ct);
        return Ok(ResponseDto.Success());
    }
}